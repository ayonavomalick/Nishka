codeunit 81852 "FOD Form Data Example"
{
    ...
    begin
        ...
        FormData.AppendLine('Content-Disposition: form-data; username=niska_bc;');
        FormData.AppendLine('Content-Type: text/plain');
        FormData.AppendLine('');
        FormData.AppendLine('bar1');

        FormData.AppendLine(StrSubstNo('--%1', Boundary));
        FormData.AppendLine('Content-Disposition: form-data; password=nimbus@123;');
        FormData.AppendLine('Content-Type: text/plain');
        FormData.AppendLine('');
        ...
        APIMgt.SendRequest(FormData.ToText(), Enum::"Http Request Type"::POST, 'http://20.233.27.118:8085/NJ/token', StrSubstNo('multipart/form-data; boundary=%1', Boundary), ResponseData);
